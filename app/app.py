...
def log_request_info():
    ...
    url = request.url
    logger.info(f'{request_ip} - - "{method} {url}"')
...
def add_user():
    ...
    try:
        ...
        if db.users.find_one({"mail": user_data["mail"]}):
            logger.warning("Attempt to create a user with an existing email")
            return jsonify({"message": "Email already exists"}), 400
        
        user_data["password"] = hash(user_data["password"])
        db.users.insert_one(user_data)
        logger.info(f"User created successfully: {user_data['mail']}")
        return jsonify({"message": "User created successfully", "user": user_schema.dump(user_data)}), 201
    except ValidationError as err:
        logger.warning(f"Validation error while creating user: {err.messages}")
...
def get_user(id):
    try:
        ...
        logger.error(f"Error while retrieving user with ID {id}: {e}")
        return jsonify({"message": str(e)}), 500
...
def update_user(id):
    ...
    try:
        ...
        if result.matched_count == 0:
            logger.warning(f"Attempt to update non-existing user with ID {id}")
            return jsonify({"message": "User not found"}), 404
        ...
        logger.warning(f"Validation error while updating user: {err.messages}")
        return jsonify(err.messages), 400
    except Exception as e:
        logger.error(f"Error while updating user with ID {id}: {e}")
        return jsonify({"message": str(e)}), 500
...
def delete_user(id):
    try:
        ...
        if result.deleted_count == 0:
            logger.warning(f"Attempt to delete non-existing user with ID {id}")
            return jsonify({"message": "User not found"}), 404
        logger.info(f"User with ID {id} deleted successfully")
        return jsonify({"message": "User deleted successfully"}), 200
    except Exception as e:
        logger.error(f"Error while deleting user with ID {id}: {e}")
        return jsonify({"message": str(e)}), 500
...
def get_cart(user_id):
    try:
        ...
        if not user:
            ...
            return jsonify({"message": "User not found"}), 404
        logger.info(f"Retrieved cart for user with ID {user_id}")
        return {"cart": user.get("cart", [])}, 200
    except Exception as e:
        logger.error(f"Error while retrieving user with ID {user_id}: {e}")
        return jsonify({"message": str(e)}), 500
...
def add_to_cart(user_id):
    try:
        ...
        logger.info(f"Article added to cart for user with ID {user_id}")
        return {"message": "Article ajouté au panier"}, 200

    except ValidationError as err:
        logger.error(f"Validation error while adding article to cart: {err.messages}")
...
def remove_from_cart(user_id, article_id):
    try:
        ...
        logger.info(f"Article with ID {article_id} removed from cart for user with ID {user_id}")
        return {"message": "Article supprimé du panier"}, 200
    except Exception as e:
        logger.error(f"Error while removing article from cart for user with ID {user_id}: {e}")
...
def clear_cart(user_id):
    try:
        ...
        if result.matched_count == 0:
            logger.warning(f"Attempt to clear cart for non-existing user with ID {user_id}")
            return {"error": "Utilisateur introuvable"}, 404
        logger.info(f"Cart cleared for user with ID {user_id}")
        return {"message": "Panier vidé"}, 200
    except Exception as e:
        logger.error(f"Error while clearing cart for user with ID {user_id}: {e}")
        return {"error": str(e)}, 500
...
def create_poll():
    try:
        ...
        logger.info(f"Poll created successfully: {created_poll['titre']}")
        return poll_schema.dump(created_poll), 201

    except ValidationError as err:
        logger.error(f"Validation error while creating poll: {err.messages}")
        return {"errors": err.messages}, 400
...
def get_polls():
    ...
    logger.info("Retrieved all polls")
...
def create_response():
    try:
        ...
        logger.info(f"Response created successfully for poll {response['poll_id']}")
        return jsonify(response_schema.dump(response)), 201
    except ValidationError as err:
        logger.error(f"Validation error while creating response: {err.messages}")
        return jsonify(err.messages), 400
...
def get_poll_responses(poll_id):
    try:
        ...
        logger.info(f"Retrieved responses for poll {poll_id}")
        return jsonify(response_schema.dump(responses)), 200
    except Exception as e:
        logger.error(f"Error while retrieving responses for poll {poll_id}: {e}")
        return jsonify({"message": str(e)}), 500
...
def create_post():
    try:
        ...
        logger.info(f"Post created successfully: {created_post['title']}")
        return post_schema.dump(created_post), 201
    except ValidationError as err:
        logger.error(f"Validation error while creating post: {err.messages}")
        return {"errors": err.messages}, 400
...
def get_posts():
    try:
        ...
        logger.error(f"Error while retrieving posts: {e}")
        return jsonify({"message": str(e)}), 500
...
def get_post(post_id):
    try:
        ...
        logger.error(f"Error while retrieving post with ID {post_id}: {e}")
        return {"error": str(e)}, 500
...
def update_post(post_id):
    try:
        ...
        logger.error(f"Validation error while updating post: {err.messages}")
        return {"errors": err.messages}, 400
    except Exception as e:
        logger.error(f"Error while updating post with ID {post_id}: {e}")
        return {"error": str(e)}, 500
...
def delete_post(post_id):
    try:
        ...
        logger.info(f"Post with ID {post_id} deleted (soft delete)")
        return {"message": "Post deleted (soft delete)"}, 200
    except Exception as e:
        logger.error(f"Error while deleting post with ID {post_id}: {e}")
        return {"error": str(e)}, 500
...
def get_active_posts():
    try:
        ...
        logger.info("Retrieved all active posts")
        return jsonify(post_schema.dump(list(posts), many=True)), 200
    except Exception as e:
        logger.error(f"Error while retrieving active posts: {e}")
        return {"error": str(e)}, 500
...
def create_article():
    try:
        ...
        logger.info(f"Article created successfully: {created_article['title']}")
        return article_schema.dump(created_article), 201
    except ValidationError as err:
        logger.error(f"Validation error while creating article: {err.messages}")
        return {"errors": err.messages}, 400
    except Exception as e:
        logger.error(f"Unexpected error while creating article: {e}")
        return {"error": str(e)}, 500
...
def get_all_articles():
    try:
        ...
        logger.error(f"Error while retrieving articles: {e}")
        return {"error": str(e)}, 500
...
def get_article(article_id):
    try:
        ...
        logger.error(f"Error while retrieving article with ID {article_id}: {e}")
        return {"error": str(e)}, 500
...
def delete_article(article_id):
    try:
        ...
        logger.error(f"Error while deleting article with ID {article_id}: {e}")
        return {"error": str(e)}, 500
